...
proc get_port_location_stats { args } {
  ...
  foreach_in_collection port $ports {
    if {[lindex [get_attribute $port user_location] $dim] > 0} {
      set loc [lindex [get_attribute $port user_location] $dim]
      set sum [expr $sum + $loc]
    } else {
      echo "hmmm... PORT does not have user_location set, its name is [get_attribute $port object_name]"
    }
    if {$loc > $max} {
      set max $loc
    }
  ...
  foreach_in_collection port $ports {
    ...
    if {$total != 0} {
      foreach_in_collection point $points {
        if {[get_attribute $point object_class] eq "pin"} {
          set cell [get_cells -of $point]
          set sum [expr $sum + [lindex [get_attribute $cell bbox] 0 $dim]]
        } elseif {[get_attribute $point object_class] eq "port"} { 
          set sum [expr $sum + [lindex [get_attribute $point user_location] $dim]]
        } else {
          echo "hmmm... POINT is not pin or port its a [get_attribute $point object_class]"
        }
      }
      set ave [expr $sum/$total]
      set diff [expr $ave - [lindex [get_attribute $port user_location] $dim]]
